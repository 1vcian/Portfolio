...
export default function ThisCantBeReached() {
  ...
  React.useEffect(() => {
    // Set ShowText after delay
    const textTimer = setTimeout(function () {
      setShowText(true);
    }, 1000);
    
    // Calculate position values on client-side only
    if (typeof window !== "undefined") {
      let heightVal = 0;
      let widthVal = 0;
      
      if(window.innerHeight > 640){
        heightVal = (window.innerHeight)/2-160-20;
      } else {
        heightVal = (window.innerHeight)/2-64-20;
      }
      
      if(window.innerWidth > 1280){
        widthVal = (window.innerWidth)/2-384-18;
      } else if(window.innerWidth > 1024){
        widthVal = (window.innerWidth)/2-192-18;
      } else if(window.innerWidth > 768){
        widthVal = (window.innerWidth)/2-144-18;
      } else if(window.innerWidth > 640){
        widthVal = (window.innerWidth)/2-96-18;
      } else {
        widthVal = (window.innerWidth)/2-16-18;
      }
      
      setCenterHeight(heightVal);
      setCenterWidth(widthVal);
    }
    ...
    >
      <div className="relative w-full  flex flex-col space-y-4">
        ...
        {/* ! Hello animation text */}
            ...
            <motion.span
            initial={{opacity:0}}
            animate={{y:CenterHeight+50,x:CenterWidth-13,opacity:1}}
        ...
        >
          ...
          <span className="text-gray-500 text-md">
            <span className="font-bold">https://1vcian.github.io/Portfolio/ </span>
            unexpectedly{" "}
            {ShowText ? (
              <motion.span
